...
INNER JOIN leads AS l
	ON l.visitor_id = v_a.visitor_id
WHERE v_a.rn = 1
GROUP BY 1, 2, 3, 4
),
...
skeleton AS(
SELECT
	visit_day, utm_source, utm_medium, utm_campaign
FROM visitors_count
	UNION
SELECT
	visit_day, utm_source, utm_medium, utm_campaign
...
LEFT JOIN visitors_count AS vc
	ON vc.visit_day = s.visit_day
	AND vc.utm_source = s.utm_source
	AND vc.utm_medium = s.utm_medium
	AND vc.utm_campaign = s.utm_campaign
LEFT JOIN ad_spent AS a_s
	ON s.visit_day = a_s.visit_day
...
LEFT JOIN leads_pu_count AS lc
	ON s.visit_day = lc.visit_day
	AND s.utm_source = lc.utm_source
	AND s.utm_medium = lc.utm_medium
	AND s.utm_campaign = lc.utm_campaign
WHERE a_s.spent != 0;
/*)
я превращал поледний этап запроса в СТЕ и вот тут вот проводил потом все вычисления
для подсчета cpu, cpl, cppu и тд.
...
SELECT
	...
	row_number() over(PARTITION BY lead_id ORDER BY s.visit_date ASC) AS rn
FROM leads AS l
...
percentiles AS (
SELECT lead_day - visit_day AS day_dif,
...
SELECT
    ...
    lag(vk_spent, 1) over(ORDER BY a.date asc) AS vk_lag_1,
    lag(ya_spent, 1) over(ORDER BY a.date asc) AS ya_lag_1,
    lag(vk_spent, 2) over(ORDER BY a.date asc) AS vk_lag_2,
    lag(ya_spent, 2) over(ORDER BY a.date asc) AS ya_lag_2
FROM organic AS o
...
SELECT
	...
	sum(count(lead_id)) over(ORDER BY lead_day - visit_day ASC) AS leads_cumulative
FROM visitors_total
WHERE rn = 1
GROUP BY 1;